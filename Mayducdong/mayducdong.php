...
elseif ($ButtonState=="Finish")
{
	...
	if ($num==1)
	{
		...
		//
		mysql_query("UPDATE mayducdong_qtsx SET date_finish ='$datetime',So_luong_lo='$So_luong_lo',So_luong_thanh='$So_luong_thanh',Trang_thai_stop='$State' WHERE ID_lohang='$ID_lohang' AND date_start ='$date_start'");
...
elseif ($ButtonState=="Pause")
{
	...
	if ($num==1)
	{
		...
		//
		mysql_query("INSERT mayducdong_tamdung (ID_lohang,date_pause,date_start,Trang_thai_start) VALUES ('$ID_lohang','$datetime
			','$date_start','$State')");
...
elseif ($ButtonState=="Resume")
{
	...
	else
	{
		...
		if($num>0)// đang Pause
		{
			...
			mysql_query("UPDATE mayducdong_tamdung SET date_resume ='$datetime',Trang_thai_stop='$State' WHERE ID_lohang='$ID_lohang' AND date_pause ='$date_pause'");
...
if ($thanhcong == 1)
{
	...
	if ($sunday=='Sunday'||((strtotime($datetime)-strtotime("$day 17:15:00")>0)))
	{
		if ($ButtonState=='Start'||$ButtonState=='Resume')
		{
			mysql_query("INSERT INTO mayducdong_tangca (ID_lohang,date_start_overtime,date_start,Trang_thai_start) VALUES ('$ID_lohang','$datetime','$date_start','$State') ");
		}
		elseif ($ButtonState=='Finish'||$ButtonState=='Pause')
		{
			...
			{
				mysql_query("UPDATE mayducdong_tangca SET date_finish_overtime ='$datetime',Trang_thai_stop='$State' WHERE ID_lohang='$ID_lohang' AND date_start ='$date_start' AND date_start_overtime LIKE '$day%' AND date_finish_overtime IS NULL");
			}
		}
